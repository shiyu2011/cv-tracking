...
def load_intrinsic(calib_path):
    ...
    K = fs.getNode("K").mat() or fs.getNode("camera_matrix").mat()
    D = fs.getNode("D").mat() or fs.getNode("distortion_coefficients").mat()
    fs.release()
    if K is None:
...
def triangulate(P1, P2, pts1, pts2):
    ...
    X_h = cv2.trangulatePoints(P1, P2, pts1_h, pts2_h)
    X = (X_h[:3] / X_h[3]).T
    return X
...
def visualize(img, pts, depth, title=""):
    ...
    colors = cv2.applyColorMap((norm*255).astype(np.unit8), cv2.COLORMAP_JET)
    for (u, v), c in (pts, colors):
...
def run_mono(img1, img2, K, D):
    pt1s1, pts2 = detect_and_match(img1, img2)
    E, mask = cv2.findEssentialMat(pts1, pts2, K, cv2.RANSAC, 0.999, 1.0)
    inlier, R, t, _ = cv2.recoverPose(E, pts1, pts2, K)
...
def main():
    ...
    ap.add_argument("--left", type=str, required=True, help="Left image path")
    ap.add_argument("--right", type=str, help="Right image path (for stereo)")
    ap.add_argument("--calib", type=str, required=True, help="Calibration file path (YAML)")
    ap.add_argument("--baseline", type=float, default=0.1, help="Baseline in meters (for stereo)")
    ap.add_argument("--show", action="store_true", help="Show the result")
    args = ap.parse_args()
